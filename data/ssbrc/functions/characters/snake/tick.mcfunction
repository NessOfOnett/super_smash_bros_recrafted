...
execute as @e[scores={snake.apm=45..}] at @s run summon minecraft:armor_stand ~ ~-1.65 ~ {Invisible:1b,Invulnerable:1b,NoBasePlate:1b,NoGravity:1b,ShowArms:1b,ArmorItems:[{},{},{},{}],HandItems:[{id:"minecraft:green_terracotta",Count:1b},{}],Pose:{RightArm:[181f,0f,0f]},Tags:["apm"]}
...
execute at @e[tag=apmPrimed] if entity @e[tag=!apmPrimed,distance=..1.5] run say boom!
execute as @e[tag=apmPrimed] at @s if entity @e[tag=!apmPrimed,distance=..1.5] run kill @s
