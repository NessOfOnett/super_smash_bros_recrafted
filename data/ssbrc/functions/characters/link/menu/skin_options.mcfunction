tellraw @s [{"text":"Unlocked Costumes:","bold":true,"color":"yellow"},{"text":"\n","bold":false},{"text":"[ ","color":"dark_green"},{"text":"Default","color":"green","clickEvent":{"action":"run_command","value":"/execute as @s[tag=!default] run function ssbrc:characters/link/menu/costumes/default"}},{"text":" ]","color":"dark_green"}]
